...
import time
import csv
from config import LOG_FILE_NAME, URL, HEADERS, date, ip_data, CSV_FILE_PATH
...
def handle_exception(err, err_type, retry_func):
    """Function to handle exceptions in a unified manner"""
...
def check_payload(now_query, offset):
    """Function to check the payload to be sent"""
    return {"query": now_query, "offset": offset}


def process_query(url, c2_name, payload, COUNT=0):
    """Function to collect IP data for the given query value"""
    global MAX_RETRY_COUNT
    if COUNT >= MAX_RETRY_COUNT:
        logging.error(
           "Maximum retry count reached. Please check the server."
        )
    ...
    try:
        ...
        logging.info(f"Number of unique IPs so far: {len(ip_data)}")
...
def process_ioc(c2_name, query_list):
    """Function to calculate the maximum execution count to check malicious tags"""
        ...
        while True:
            ...
            try:
                ...
                for count in range(total_count):
                    ...
                    if offset > 9900:
                        logging.error(
                            "Reached the maximum offset value, proceeding to the next query."
                        )
                        break
                    payload = check_payload(now_query, offset)
                    time.sleep(RETRY_DELAY_SECONDS)
                    process_query(URL, c2_name, payload)
