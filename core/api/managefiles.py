...
def find_unique_ip_addresses(today_filename, yesterday_filename):
    """Function to find unique IP addresses"""
...
def write_ip_addresses_to_csv(ip_addresses, filename):
    """Function to save IP address data to a CSV file"""
...
def filter_old_data_and_get_ip_addresses(file_path, SEVEN_DAYS_AGO):
    """Function to extract a list of IP addresses older than one week"""
    try:
        ip_addresses = []
...
def extract_and_save_to_json(DELETE_TEMP_CSV_FILE_NAME, DELETE_TEMP_JSON_FILE_NAME):
    """Function to save extracted data of IPs to a JSON file for deletion"""
    try:
        extracted_data = {}
...
def merge_files_and_create_nextday_file(
    CREATE_TEMP_CSV_FILE_NAME, yseterday_csv_file, nextday_csv_file
):
    """Function to merge files and create a new file reflecting updated additions and deletions of IPs for single-file management"""
    try:
        ...
        if not os.path.exists(
            ...
            try:
                shutil.copy2(yseterday_csv_file, nextday_csv_file)
                logging.info(
                    f"CREATE_TEMP_CSV_FILE_NAME does not exist. Copied YESTERDAY_CSV_FILE_PATH to NEXTDAY_CSV_FILE_PATH."
                )
                return

            except Exception as e:
                logging.error(f"Error in copying file: {str(e)}")
            ...
            if os.path.exists(yseterday_csv_file):
                with open(yseterday_csv_file, "r", newline="") as yesterday_file:
                    yesterday_reader = csv.DictReader(yesterday_file)
                    for row in yesterday_reader:
                        if (
                            "Date" in row
                            and datetime.strptime(row["Date"], "%Y-%m-%d") >= sevenday
                        ):
                            combined_data.append(row)
            ...
            logging.info(
                f"Files merged and next day file {nextday_csv_file} created successfully."
            )
...
def delete_files_in_folder(folder_path, except_files=None):
    """Function to filter out files to exclude and delete them from a folder"""
    logging.info(f"File to remain: {except_files}")
...
def remove_file_with_log(file_name):
    ...
    if check_file_existence(file_name):
        os.remove(file_name)
        logging.info(f"{file_name} file is deleted.")
    else:
        logging.info(f"{file_name} file does not exist. It's not deleted.")
